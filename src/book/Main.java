...
//				System.out.println(i+" : "+link.get(i));
			for (int i = 500; i < 559; i++) {// 560부터 음반 및 잡것
				...
				for (int p = 1; p < 10; p++) {
					Document doc1 = Jsoup.connect(
							"https://www.aladin.co.kr" + link.get(i).attr("href") + "&BrowseTarget=List&page=" + p)
							.get();
					// System.out.println("현재 찾는 주소 :
					// "+"https://www.aladin.co.kr"+link.get(i).attr("href")+"&BrowseTarget=List&page="+p);
					// System.out.println("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
					...
					for (int j = 0; j < detailLinks.size(); j++) {
						try {
							...
							System.out.print("카테고리 : "+i+" / 페이지 : "+p+" / 번호 : "+j);
							vo.setTitle(title);
							...
							if (m.find())
								...
								System.out.println(" -> isbn형식 오류(건너뜀)");
								continue;
							}
							...
							if (dao.isIsbn(Long.parseLong(isbn))) {
								...
								System.out.println(" / 제목 : " + title +" -> 수집");
							...
							///////////////////////////////////////////////////////////
							Document yes0 = Jsoup
									.connect("http://www.yes24.com/SearchCorner/Search?domain=BOOK&query=" + isbn)
									.get();
				...
				// 한카테고리당 태그 수집 부분
				System.out.println("한 카테고리 끝나고 태그 수집 부분~!");
				// 데이터 수집 시작할때만 주석 풀고, 수집 후 다시 주석하기
				dao.tagInsert(set);
